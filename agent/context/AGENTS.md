...
- Preserve concrete links in the final output for these multi-story newsletters exactly: 1440, The Objective, The Substack Post, TLDR, Superhuman, HackerNoon, and Xataka.
- Treat Xataka as an articles newsletter as well: keep each article title, include a brief description when available, and keep one `Link` for each listed item when available.
